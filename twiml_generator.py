...
def create_phone_tree_navigation_twiml():
    """
    Create TwiML that navigates phone tree and records full greeting.
    
    Legacy function - used for simple phone tree navigation where we always press 1.
    """
    response = VoiceResponse()
    
    # NO PAUSE - Start immediately to capture disclaimer that plays at call start
    # The disclaimer is legally required and plays IMMEDIATELY when call connects
    
    # Speak friendly message (if human picks up, they'll hear this)
    response.say(
        Config.HUMAN_MESSAGE,
        voice='Polly.Joanna',
        language='en-US'
    ...
    # Wait for phone tree menu (10 seconds)
    response.pause(length=10)
    
    # Press 1 for Leasing (we always want leasing)
    response.play(digits='1')
    response.pause(length=2)
    
    # Wait for next menu or voicemail (10 more seconds)
    # Wait and listen (call-level recording captures everything)
